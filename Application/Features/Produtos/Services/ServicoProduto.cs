...
namespace DesafioMirante.Application.Features.Produtos.Services;
...
public sealed class ServicoProduto : IServicoProduto
{
        ...
        CancellationToken cancellationToken)
    {
        _logger.LogInformation(
            "Consultando produtos paginados. Pagina: {NumeroPagina}, Tamanho: {TamanhoPagina}, Nome: {Nome}, Ativo: {Ativo}",
            filtro.NumeroPagina,
    ...
    {
        _logger.LogInformation("Consultando produto {ProdutoId}", id);
    ...
    private static void NormalizarRequisicao(RequisicaoCriarProduto requisicao)
    {
        requisicao.Nome = requisicao.Nome.Trim();
        requisicao.Sku = requisicao.Sku.Trim().ToUpperInvariant();
        requisicao.Descricao = string.IsNullOrWhiteSpace(requisicao.Descricao)
            ? null
            : requisicao.Descricao.Trim();
    }

    private static void NormalizarRequisicao(RequisicaoAtualizarProduto requisicao)
    {
        requisicao.Nome = requisicao.Nome.Trim();
        requisicao.Sku = requisicao.Sku.Trim().ToUpperInvariant();
        requisicao.Descricao = string.IsNullOrWhiteSpace(requisicao.Descricao)
